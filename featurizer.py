"""A feature extractor for crfsuite"""
import crfutils, sys, os, re
import string
import sqlite3
import json
import requests

# Separator of field values.
separator = '\t'
templates = []
fields = 'w y'

templates = (
    (('w', -1), ),
    (('w',  0), ),
    (('w', -1), ('w',  0)), 
    (('w',  0), ('w',  1)),
    )

DF = None
class DictionaryFeatures:
    def __init__(self, dictDir):
        self.word2dictionaries = {}
        self.dictionaries = []
        i = 0
        self.con = sqlite3.connect("/cygdrive/d/iiit/title_cat.db")
        for d in os.listdir(dictDir):
            print >> sys.stderr, "read dict %s"%d
            if d == '.svn':
                continue
            self.dictionaries.append(d)
            for line in open(dictDir + "/" + d):
                word = line.rstrip('\n')
                word = word.strip(' ').lower()
                if not self.word2dictionaries.has_key(word):            
                    self.word2dictionaries[word] = str(i)
                else:
                    self.word2dictionaries[word] += "\t%s" % i
            i += 1
        
    MAX_WINDOW_SIZE=6
    
    def getDBPediaLookup(self,term):
        features = []
        params = {}
        params['QueryString'] = term
        headers = {'Accept': 'application/json'}
        url = "http://localhost:1111/api/search/KeywordSearch"
        #print >> sys.stderr, "Lookup DBPEDIA : " + term
        response = requests.get(url,params=params,headers=headers)

        data = response.json()
         

        for i in range(0,len(data['results'])):
            dataItem = data['results'][i]
            classes = dataItem['classes']
            for j in range(0,len(classes)):
                classLabel = classes[j]['label']
                features.append(classLabel.replace(" ","_").encode('ascii', 'ignore'))
            categories = dataItem['categories']
            for k in range(0,len(categories)):
                cateLabel  = categories[k]['label']
                features.append(cateLabel.replace(" ","_").encode('ascii', 'ignore'))
        return list(set(features))
    
    def GetDBFeatures(self,phrase):
        features = []
        query = "Select * from title_category where title = ?"
        query1= "select category from redirects_title A, title_category B where A.title=B.title and A.redirect_title = ?"; 
        #print >> sys.stderr, "Executing "+phrase
        cursor = self.con.execute(query,(phrase,))
        data = cursor.fetchall()
        for d in data:
            features.append(str(d[1]));
        cursor1 = self.con.execute(query1,(phrase,))
        data1 = cursor1.fetchall()
        for d in data1:
            features.append(str(d[0]));
        return list(set(features))
    
    def GetDictFeatures(self, words, i):
        #print words + "  " + str(i)
        features = []
        for window in range(self.MAX_WINDOW_SIZE):
            for start in range(max(i-window+1, 0), i+1):
                end = start + window
                #print str(start) +"  "+str(end)+"  "+str(window)
                phrase = ' '.join(words[start:end]).lower().strip(string.punctuation)
                phraseForDB = '_'.join(words[start:end]).lower().strip(string.punctuation)
                dbFeatures = []
                if phraseForDB:
                    dbFeatures = self.getDBPediaLookup(phrase)
                    dbFeatures = dbFeatures + self.GetDBFeatures(phraseForDB)
                if self.word2dictionaries.has_key(phrase):
                    #print self.word2dictionaries[phrase]
                    for j in self.word2dictionaries[phrase].split('\t'):
                        features.append('DICT=%s' % self.dictionaries[int(j)])
                        if window > 1:
                            features.append('DICTWIN=%s' % window)
                if dbFeatures:
                    #print >> sys.stderr, "Adding "+dbFeatures
                    features = features+dbFeatures
                    if window > 1:
                            features.append('DICTWIN=%s' % window)
        return list(set(features))

def GetOrthographicFeatures(word, goodCap=True):
    features = []

    features.append("word=%s" % word)
    features.append("word_lower=%s" % word.lower())
    if(len(word) >= 4):
        features.append("prefix=%s" % word[0:1].lower())
        features.append("prefix=%s" % word[0:2].lower())
        features.append("prefix=%s" % word[0:3].lower())
        features.append("suffix=%s" % word[len(word)-1:len(word)].lower())
        features.append("suffix=%s" % word[len(word)-2:len(word)].lower())
        features.append("suffix=%s" % word[len(word)-3:len(word)].lower())

    if re.search(r'^[A-Z]', word):
        features.append('INITCAP')
    if re.search(r'^[A-Z]', word) and goodCap:
        features.append('INITCAP_AND_GOODCAP')
    if re.match(r'^[A-Z]+$', word):
        features.append('ALLCAP')
    if re.match(r'^[A-Z]+$', word) and goodCap:
        features.append('ALLCAP_AND_GOODCAP')
    if re.match(r'.*[0-9].*', word):
        features.append('HASDIGIT')
    if re.match(r'[0-9]', word):
        features.append('SINGLEDIGIT')
    if re.match(r'[0-9][0-9]', word):
        features.append('DOUBLEDIGIT')
    if re.match(r'.*-.*', word):
        features.append('HASDASH')
    if re.match(r'[.,;:?!-+\'"]', word):
        features.append('PUNCTUATION')
    return features

def Featurizer(X):
    global DF
    #print X
    if X:
        allWords = []
        for t in range(len(X)):
            allWords.append(X[t]['w']) 
        for t in range(len(X)):
            w = X[t]['w']
            #feats = DF.GetDictFeatures(allWords,t) + GetOrthographicFeatures(w)
            feats = DF.GetDictFeatures(allWords,t) + GetOrthographicFeatures(w)
            for f in feats:
                X[t]['F'].append('%s'%(f))

def FeatureExtractor(X):
    """apply attribute templates to obtain features (in fact, attributes)"""
    crfutils.apply_templates(X, templates)
     
    Featurizer(X)
    #print >> sys.stderr, X
    if X:
        X[0]['F'].append('__BOS__')     # BOS feature
        X[-1]['F'].append('__EOS__')    # EOS feature

if __name__ == '__main__':
    DF = DictionaryFeatures("./lexicon")
    crfutils.main(FeatureExtractor, fields=fields, sep=separator)
    DF.con.close()
